...
class doUITemp:
    def __init__(self, func, case, caseFile):
        self.func = func
        self.case = case
    ...
    def result(self, flag, action):
        if flag:
            return f"File {action} Successfully", self.expectValue, self.title
        else:
            return f"File {action} Failed", self.expectValue, self.title
    
    def goBack(self, NOTZORE=True):
    ...
    def singleFileRemove(self):
        ...
        if importNumber == removeNumber + removeNum:
            return True
        else:
            return False
    ...
    def fileRepair(self):
        ...
        if importNumber == int(successNum):
            return True
        else:
            return False
    ...
    def fileHighRepair(self):
        ...
        # 高级修复功能
        if highRepair(self.app, self.highRepairPath, failNum):
            return True
        else:
            return False
    ...
    def filePreview(self):
        ...
        # 获取当前播放进度, 时间与time.sleep(3)一致
        if BasePage(self.app).get_control_title("window_re", "00:00:03") == "00:00:03":
            return True
        else:
            return False
    ...
    def singleFileExport(self):
        ...
        # 等待导出完成
        result = waitExportComplete(self.app, isSingleExport=True)
